...
public class UserController {
    ...
    public ResponseEntity<ApiResponseDto<List<ReservationDto>>> getMyReservations(
            @AuthenticationPrincipal CustomUserDetails userDetails
    ) {
        String userEmail = userDetails.getUsername();
        List<ReservationDto> reservationDtos = userService.getUserReservations(userEmail);
        ApiResponseDto<List<ReservationDto>> responseDto = ApiResponseDto.of(ResponseSuccessCode.RESERVATION_INFO, reservationDtos);
        return ResponseEntity.status(HttpStatus.OK).body(responseDto);
    }
